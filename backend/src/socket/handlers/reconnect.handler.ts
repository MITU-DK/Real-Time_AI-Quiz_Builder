...
export const handleReconnect = (namespace: GameNamespace, socket: GameSocket) => {
  ...
  socket.on('rejoin_room', async ({ playerId, pin }) => {
    try {
      // 1. Verify the player ID still exists in the Redis presence set (game is still live)
      const isMember = await redis.sismember(`room:${pin}:presence`, playerId.toString());
      ...
      // 2. Re-register the new socket ID in Redis for this player

      //get (Player details + Current quiz question) in single query.
      const participantResult = await pool.query(
        `SELECT p.id, p.nickname, p.score, gs.current_question_index
      ...
      const { nickname, score, current_question_index } = participantResult.rows[0];

      //reconnect usually creates a new socket.
      // 3. Update the socket-to-player mapping with the new socket ID
      await Promise.all([
