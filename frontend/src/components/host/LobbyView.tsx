    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex flex-col">
      ...
      <div className="bg-white/80 backdrop-blur border-b border-slate-100 p-4">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <h1 className="text-xl font-bold text-blue-700 font-[Outfit]">⚡ QuizArena</h1>
          <button
            onClick={() => navigate('/dashboard')}
            className="text-sm text-slate-500 hover:text-blue-600 font-medium transition cursor-pointer flex items-center gap-1"
          >
            🏠 Home
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8">
        ...
        <div className="flex items-center gap-4">
          <button
            onClick={() => navigate('/dashboard')}
            className="px-8 py-4 bg-white hover:bg-slate-50 text-slate-600 text-lg font-bold rounded-2xl transition-all shadow-md border border-slate-200 hover:border-slate-300 cursor-pointer"
          >
            ✕ Cancel
          </button>
          <button
